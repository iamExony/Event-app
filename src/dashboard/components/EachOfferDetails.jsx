...
const EachOfferDetails = ({ type, data, options, isOpen, onOpen }) => {
  const { id, event_id, date, source, location, status, client } = data;
  // const { name } = client;
  console.log({ data });

  // formatedDate = formatDateAndTime(date)

  // Pending and converted styles
  // const pending = document.addEventListener("click", (e) => {
  ...
  return (
    <tr className="bg-white border-b  hover:bg-gray-50 w-full">
      ...
      <td className="px-6 py-4 whitespace-nowrap">{formatDateAndTime(date)}</td>
      <td className="px-6 py-4">{data?.event?.client?.name}</td>
...
export default EachOfferDetails;

...
function SingleSpecReceipt({ specification }) {
  //Formating number
  function formatNumber(num) {
    return Intl.NumberFormat("en-GB").format(num);
  }

  let subTotalAmount = Number(specification.amount);

  let VAT = 0.075 * subTotalAmount;
  let totalAmount = formatNumber(subTotalAmount + VAT);

  return (
    <div className="flex  flex-col  border-2 border-primaryCol w-[70%] m-auto mt-16 py-6 relative">
      <div className="pl-6 flex justify-between items-end">
        <div className="">
          <Logo className="" />
        </div>
        <div className="uppercase font-bold text-4xl text-primaryCol border-b-4 border-primaryCol w-[250px] text-right pr-6 ">
          Invoice
        </div>
      </div>
      <div className=" my-12 grid grid-cols-2 px-6">
        <div className="grid grid-cols-1 justify-items-start gap-2 w-[130%]">
          <p>
...
export default SingleSpecReceipt;
